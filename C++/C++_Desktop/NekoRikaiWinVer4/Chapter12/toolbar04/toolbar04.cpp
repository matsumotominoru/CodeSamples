...
HWND MyCreateToolbar(HWND hWnd)
{
    ...
    hTool = CreateWindow(TOOLBARCLASSNAME, NULL, WS_CHILD | WS_VISIBLE, 
        0, 0, 0, 0, hWnd, (HMENU)ID_TOOLBAR, hInst, NULL);

    SendMessage(
        hTool, TB_BUTTONSTRUCTSIZE, (WPARAM)sizeof(TBBUTTON), 0);
    tbab.hInst = NULL;    //自作ビットマップのときはNULL
    tbab.nID = (int)LoadImage(hInst,  MAKEINTRESOURCE(IDR_TOOLBAR1),
        IMAGE_BITMAP,  0, 0, LR_LOADMAP3DCOLORS | LR_SHARED);
    SendMessage(hTool, TB_ADDBITMAP, (WPARAM)3, (LPARAM)&tbab);

    LoadString(hInst, IDS_MARU, szBuf, (int)sizeof(szBuf));
    iMaru = SendMessage(hTool, TB_ADDSTRING, 0, (LPARAM)szBuf);
    tbb[0].iString = iMaru;

    LoadString(hInst, IDS_SANKAKU, szBuf, (int)sizeof(szBuf));
    iSankaku = SendMessage(hTool, TB_ADDSTRING, 0, (LPARAM)szBuf);
    tbb[1].iString = iSankaku;

    LoadString(hInst, IDS_SHIKAKU, szBuf, (int)sizeof(szBuf));
    iShikaku = SendMessage(hTool, TB_ADDSTRING, 0, (LPARAM)szBuf);
    tbb[2].iString = iShikaku;

    SendMessage(
        hTool, TB_ADDBUTTONS, (WPARAM)3, (LPARAM)(LPTBBUTTON)&tbb[0]);
